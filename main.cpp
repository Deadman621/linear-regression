...
int main(void) {
    ...
    try {
        ...
        Data data{"C:\\Users\\tech\\Desktop\\OPP_Project\\linear-regression\\Datasets\\Salary_Data.csv"};
        ...
        tuple<double, double , int> best_hyperparameters = optimizer.GridSearch(LearningRate_Values, epochs_values, data.getTrainingDataPoints(true,false),true,false);  // true for Normalization and false for Standardization
        model.SetLearningRate(get<0>(best_hyperparameters));
        model.Train(get<1>(best_hyperparameters) ,get<2>(best_hyperparameters), false);  
